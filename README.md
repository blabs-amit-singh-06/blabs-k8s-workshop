...
# step 2:minikube dashboard  (to get the ui of dash board)
# step 3:minikube stop
...
#      1.kubectl apply -f cluster_service/service.yaml  ** deploy a service
#      2.kubectl get services  or  kubectl get svc
#      3.kubectl port-forward service/k8s-workshop-clusterip 9080:5000

#      NodePort : it is directly exposing the pods to real world( not considered safe)
#      1.kubectl apply -f node/service.yaml  ** deploy a service
#      2.kubectl get services  or  kubectl get svc

#      loadbalancer: it is used to expose the pods to public domain
#      1.kubectl apply -f loadbalancer/service.yaml  ** deploy a service
